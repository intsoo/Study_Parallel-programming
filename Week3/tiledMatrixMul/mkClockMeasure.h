...
class mkClockMeasure {
 ...
 public:
  ...
  void clockPrint() {
    ...
    if (time < 1000) {
      printf("Total Time: \t%10lf (ns), \tAvg Time: \t%10lf (ns), \tCount: \t%ld",
	     (double)time, (double)time / count, count);
    } else if (time < 1000000) {
      printf("Total Time: \t%10lf (us), \tAvg Time: \t%10lf (us), \tCount: \t%ld",
	     (double)time / 1000, (double)time / (1000 * count), count);
    } else if (time < nanoToSecTime) {
      printf("Total Time: \t%10lf (ms), \tAvg Time: \t%10lf (ms), \tCount: \t%ld",
	     (double)time / 1000000, (double)time / (1000000 * count), count);
    } else {
      printf("Total Time: \t%10lf (s), \tAvg Time: \t%10lf (s), \tCount: \t%ld",
